...
dir.create('c:/Drive/Data/Merino/A/Pob/2023/Resultados') #ACTUALIZAR
Result <- ('c:/Drive/Data/Merino/A/Pob/2023/Resultados') #ACTUALIZAR
...
dashA <- todo.j3[,c('RIM','CAMPO','NOMBRE','ident','rp_padre','MANEJO','AN.x','SEXO','B','PCD','PC1','PVS1','PDF1','depPCD','depPC1','depPVL1','depPDF1',
                    'ind02','ind10','exa','vcPCD','vcPC1','vcPVL1','vcPDF1')]

dashA <- subset(dashA,AN.x>=2000)
dashA$ANIO <- paste("01","/","01","/",dashA$AN.x,sep="")
dashA <- dashA[,-7]
...
#Listados de padres con filtros sin Pilca ni RM para para exportar a cat?logo de padres AACM
...
padresA <- padres1[,c('RIM','NOMBRE','AN','rp_padre','depPCD','depPVL1','depPDF1','depPC2','depCVF1','depLM1','depRT1','ind10','ind02','exa','NT','NC','con')]
padresA$B <- "Astado"
padresA$ANIO <- paste("01","/","01","/",padresA$AN,sep="")
padresA <- padresA[,-3]
...
TG.pobA <- aggregate(cbind(vcPCD,vcPC1,vcPC2,vcPVL1,vcPDF1,vcCVF1,vcRT1,vcLM1,ind10,ind02) ~ AN, FUN=mean, 
                    subset = AN >= an.tg, data=todo.j2)

plot(TG.pobA$AN,TG.pobA$ind10)
lines(TG.pobA$AN,TG.pobA$ind10)

write.table(TG.pobA, file = "TendenciaA_pob.csv", sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)
...
web <- todo.j3[,c('CAMPO','RIM','PVS2','RIN2','PVL2','PDF2','CVF2','FC2','PC2','PC1','PVS1','PVL1','PDF1',
...
web <- subset(web, !CAMPO=='Pilc'& !CAMPO=='RioM')
web <- web[,-1]
...
#madres <- subset(todo.j3,todo.j3$CAMPO==est) #tambien se podr?a filtrar por las madres de la ultima generacion
...
rm(pad.est,padres3,prog2D.est,prog4D.est)
