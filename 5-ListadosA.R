...
dir.create('c:/Drive/Data/Merino/A/Pob/2022/Resultados') #ACTUALIZAR
Result <- ('c:/Drive/Data/Merino/A/Pob/2022/Resultados') #ACTUALIZAR
...
per.int <-1999   #Período de interes > a...
dec <- ','       #AACM lo quiere con ','

#Esta parte correrla cada vez que se corra para establecimiento
est <-  'Pilc'   #Campo a listar

out <- paste('PadresA_',est,'.csv',sep='')
out2 <- paste('Prog.A.2D_',est,'.csv',sep='')
out3 <- paste('Prog.A.4D_',est,'.csv',sep='')
out4 <- paste('Tendencia.A_',est,'.csv',sep='')
out5 <- paste('TablaA1_',est,'.csv',sep='')
out6 <- paste('TablaA2_',est,'.csv',sep='')
out7 <- paste('TablaA3_',est,'.csv',sep='')
out8 <- paste('TablaA4_',est,'.csv',sep='')
out9 <- paste('TablaA5_',est,'.csv',sep='')
out10 <- paste('Prog.A.6D_',est,'.csv',sep='')
out11 <- paste('MadresA_',est,'.csv',sep='')
...
todo.j3$MANEJO <- as.factor(ifelse(todo.j3$MANEJO=='cabaña'|todo.j3$MANEJO=='galpón','SI','NO'))
...
tx$NT <- tx$Cabo+tx$Cris+tx$Lele+tx$Mana+tx$RioP+tx$Teck+tx$Pilc+tx$RioM+tx$CerC
...
tx2 <- tx
tx2[tx2!=0] <-1
tx2$NC <- tx2$Cabo + tx2$Cris + tx2$Lele + tx2$Mana + tx2$RioP + tx2$Teck + tx2$Pilc + tx2$RioM +tx2$CerC
...
#Listados de padres con filtros sin Pilca ni RM para para exportar a catálogo de padres AACM
padres1 <- merge(todo.j3,tx3,by='RIM',all.y=T)
padres1 <- subset(padres1,padres1$exa>=minexa & padres1$NT>=minnt)
padres1 <- subset(padres1, is.na(padres1$CAMPO) | (padres1$CAMPO!='Pilc' & padres1$CAMPO!='RioM'))
padres1 <- subset(padres1, !is.na(padres1$NOMBRE))
...
write.table(padres, file = "PadresA_pob.csv", sep=";", row.names = F, col.names = T, quote=F, na="0", dec=dec)


#Listados de PADRES sin Pilca ni RM para informes individuales
#padres2 <- merge(todo.j3,tx3,by='RIM',all.y=T)
#padres2$CAMPO.p[is.na(padres2$CAMPO.p)] <- padres2$CAMPO
#padres2 <- subset(padres2,padres2$CAMPO!='Pilc' & padres2$CAMPO!='RioM')

#Listado de PROGENIE poblacion para exportar en ese orden a merino.ogr.ar AACM;
progen <- subset(todo.j3,todo.j3$AN==ult.camada & todo.j3$CAMPO!='Pilc' & todo.j3$CAMPO!='RioM' & !is.na(todo.j3$CAMPO) & !is.na(todo.j3$PDF1))
...
write.table(progen, file = "Prog.A.2D_pob.csv", sep=";", row.names = F, col.names = T, quote=F, na="0", dec=dec)

#Tendencia poblacion para exportar a catálogo de padres AACM
tend <- subset(todo.j3, todo.j3$AN>per.int)  #aca defino que años incluir en la tend genetica
t1 <- round(tapply(tend$vcPCD,tend$AN, mean, na.rm=T),digits =2)
t2 <- round(tapply(tend$vcPC1,tend$AN, mean, na.rm=T),digits =2)
t3 <- round(tapply(tend$vcPC2,tend$AN, mean, na.rm=T),digits =2)
t4 <- round(tapply(tend$vcPVL1,tend$AN, mean, na.rm=T),digits =2)
t5 <- round(tapply(tend$vcPDF1,tend$AN, mean, na.rm=T),digits =2)
t6 <- round(tapply(tend$vcCVF1,tend$AN, mean, na.rm=T),digits =2)
t7 <- round(tapply(tend$vcRT1,tend$AN, mean, na.rm=T),digits =2)
t8 <- round(tapply(tend$vcLM1,tend$AN, mean, na.rm=T),digits =2)
t9 <- round(tapply(tend$ind10,tend$AN, mean, na.rm=T),digits =0)
t10 <- round(tapply(tend$ind02,tend$AN, mean, na.rm=T),digits =0)

tendencia <- as.data.frame(cbind(t1,t2,t3,t4,t5,t6,t7,t8,t9,t10))
tendencia$AN <- rownames(tendencia)
names(tendencia)=c("vcPCD",'vcPC1',"vcPC2","vcPVL1","vcPDF1","vcCVF1","vcRT1","vcLM1",'ind10','ind02','AN')
tendencia <- tendencia[,c(ncol(tendencia),1:(ncol(tendencia)-1))] #reordeno las columnas

plot(tendencia$AN,tendencia$ind10)
lines(tendencia$AN,tendencia$ind10)

write.table(tendencia, file = "TendenciaA_pob.csv", sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)

rm(t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,tend,padres1,padres)

#Tabla: Estructura de datos poblacion Merino Astado
pob1 <- subset(todo.j3,todo.j3$AN>per.int & !is.na(todo.j3$PDF1))

pob1$SEXO <- droplevels(as.factor(pob1$SEXO))
pob1$MANEJO <- droplevels(as.factor(pob1$MANEJO))
pob1$CRIANZA <- droplevels(as.factor(pob1$CRIANZA))
pob1$rep <- droplevels(as.factor(pob1$rep))


t10 <- table(pob1$AN,pob1$SEXO)
t11 <- table(pob1$AN,pob1$CRIANZA)
t12 <- table(pob1$AN,pob1$MANEJO:pob1$SEXO)
...
write.table(tabla1, file = "TablaA1_pob.csv", sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)

rm(t10,t11,t12,t13,t14,tabla1,tendencia,progen,pob1)


#Tabla: Progenie por padre por año en poblacion Merino Astado
#pob2 <- subset(todo.j3,todo.j3$AN>(per.int+10) & !is.na(todo.j3$PDF1) & todo.j3$CAMPO!='Pilc' & todo.j3$CAMPO!='RioM'& !is.na(todo.j3$CAMPO))
pob2 <- subset(todo.j3,todo.j3$AN>(per.int) & !is.na(todo.j3$PDF1) & todo.j3$CAMPO!='Pilc' & todo.j3$CAMPO!='RioM'& !is.na(todo.j3$CAMPO))
...
write.table(tabla2, file = "TablaA2_pob.csv", sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)

rm(tabla2,t15,pob2)

#Percentiles

progen2 <- subset(todo.j3,todo.j3$AN==ult.camada & todo.j3$CAMPO!='Pilc' & todo.j3$CAMPO!='RioM' & !is.na(todo.j3$CAMPO) & !is.na(todo.j3$PDF1))
...
write.table(percentiles, file = "Percentiles.csv", sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)

#Tabla: Promedio de borregos POB manejados a campo por año
...
write.table(tabla3p, file = 'TablaA3_pob.csv', sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)
...
#Tabla: Promedio de borregos POB suplementados por año
...
write.table(tabla4p, file = 'TablaA4_pob.csv', sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)

rm(N,PC1,PVS1,RIN1,PVL1,PDF1,CVF1,FC1,LM1,RT1,tabla4p)

#Tabla: Promedio de borregas por año
...
tabla5p <- as.data.frame(cbind(N,PC1,PVS1,RIN1,PVL1,PDF1,CVF1,LM1,RT1))
tabla5p$AN <- rownames(tabla5p)
tabla5p <- tabla5p[,c(ncol(tabla5p),1:(ncol(tabla5p)-1))] #reordeno las columnas
write.table(tabla5p, file = 'TablaA5_pob.csv', sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)
...
write.table(web, file = 'webA.csv', sep=";", row.names = F, col.names = T, quote=F, na="", dec=dec)


rm(N,PC1,PVS1,RIN1,PVL1,PDF1,CVF1,LM1,RT1,tabla5p)
...
prog2D.est <- subset(todo.j3,todo.j3$AN==ult.camada & todo.j3$CAMPO==est & !is.na(todo.j3$CAMPO) & !is.na(todo.j3$PDF1))
prog2D.est <- prog2D.est[,c('NOMBRE','SEXO','CRIANZA','MANEJO','PCD','PC1','PVS1','PVL1','PDF1','CVF1','RIN1','FC1','LM1','RT1', 
                    'depPCD','depPC1','depPC2','depPVL1','depPDF1','depLM1','depRT1','ind10','ind02','exa','con')]
prog2D.est <- prog2D.est[order(prog2D.est$NOMBRE),]

write.table(prog2D.est, file = out2, sep=";", row.names = F, col.names = T, quote=F, na="0", dec=dec)

#Listado de progenie 4D de campo
prog4D.est <- subset(todo.j3,todo.j3$AN==ult.camada-1 & todo.j3$CAMPO==est & !is.na(todo.j3$CAMPO) & !is.na(todo.j3$PDF1))
prog4D.est <- prog4D.est[,c('NOMBRE',"SEXO",'CRIANZA','MANEJO','PCD','PC1','PVS1','PVL1','PDF1','CVF1','RIN1','FC1','LM1','RT1',
                            'depPCD','depPC1','depPC2','depPVL1','depPDF1','depLM1','depRT1','ind10','ind02','exa','con')]
prog4D.est <- prog4D.est[order(prog4D.est$NOMBRE),]

write.table(prog4D.est, file = out3, sep=";", row.names = F, col.names = T, quote=F, na="0", dec=dec)

#Listado de machos 6D de campo (SOLO PILCA)
#prog6D.est <- subset(todo.j3,todo.j3$AN==ult.camada-2 & todo.j3$CAMPO==est & !is.na(todo.j3$CAMPO) & !is.na(todo.j3$PDF1) & todo.j3$SEXO=='M')
#prog6D.est <- prog6D.est[,c('NOMBRE',"SEXO",'CRIANZA','MANEJO','PCD','PC1','PVS1','PVL1','PDF1','CVF1','RIN1','FC1','LM1','RT1',
#                            'depPCD','depPC1','depPC2','depPVL1','depPDF1','depLM1','depRT1','ind10','ind02','exa','con')]
#prog6D.est <- prog6D.est[order(prog6D.est$NOMBRE),]

#write.table(prog6D.est, file = out10, sep=";", row.names = F, col.names = T, quote=F, na="0", dec=dec)


#Listado de madres (opcional)
#madres <- subset(todo.j3,todo.j3$CAMPO==est) #tambien se podría filtrar por las madres de la ultima generacion
#madres <- as.data.frame(unique(madres$rimmd))
#colnames(madres) <- 'RIM'
#madres <- subset(madres, madres$RIM!=0)

#madres2 <- merge(madres,todo.j3,by='RIM',all.x=T)

#mad.est <- madres2[,c('RIM','NOMBRE','AN','rp_padre','depPCD','depPC1','depPC2','depPVL1','depPDF1','depCVF1','depLM1','depRT1','ind10','ind02','exa','con')]
#mad.est <- mad.est[order(mad.est$ind02, decreasing = T),]

#write.table(mad.est, file = out11, sep=";", row.names = F, col.names = T, quote=F, na="0", dec=dec)


#Listado tendencia de establecimiento
tend.est <- subset(todo.j3, todo.j3$AN>per.int & todo.j3$CAMPO==est)  #aca defino que años incluir en la tend genetica
t1 <- round(tapply(tend.est$vcPCD,tend.est$AN, mean, na.rm=T),digits = 2)
t2 <- round(tapply(tend.est$vcPC1,tend.est$AN, mean, na.rm=T),digits = 2)
t3 <- round(tapply(tend.est$vcPC2,tend.est$AN, mean, na.rm=T),digits = 2)
t4 <- round(tapply(tend.est$vcPVL1,tend.est$AN, mean, na.rm=T),digits = 2)
t5 <- round(tapply(tend.est$vcPDF1,tend.est$AN, mean, na.rm=T),digits = 2)
t6 <- round(tapply(tend.est$vcCVF1,tend.est$AN, mean, na.rm=T),digits = 2)
t7 <- round(tapply(tend.est$vcRT1,tend.est$AN, mean, na.rm=T),digits = 2)
t8 <- round(tapply(tend.est$vcLM1,tend.est$AN, mean, na.rm=T),digits = 2)
t9 <- round(tapply(tend.est$ind10,tend.est$AN, mean, na.rm=T),digits = 1)
t10 <- round(tapply(tend.est$ind02,tend.est$AN, mean, na.rm=T),digits = 1)

tendencia.est <- as.data.frame(cbind(t1,t2,t3,t4,t5,t6,t7,t8,t9,t10))
tendencia.est$AN <- rownames(tendencia.est)
names(tendencia.est)=c("vcPCD",'vcPC1',"vcPC2","vcPVL1","vcPDF1","vcCVF1","vcRT1","vcLM1",'ind10','ind02','AN')
tendencia.est <- tendencia.est[,c(ncol(tendencia.est),1:(ncol(tendencia.est)-1))] #reordeno las columnas

write.table(tendencia.est, file = out4, sep=";", row.names = F, col.names = T, quote=F, na=".", dec=dec)

plot(tendencia.est$AN,tendencia.est$ind10)
lines(tendencia.est$AN,tendencia.est$ind10)

rm(t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,tend.est,pad.est,padres3,prog2D.est,prog4D.est)

#Tabla: Progenie por padre por año en establecimiento
est2 <- subset(todo.j3,todo.j3$AN>(per.int) & !is.na(todo.j3$PDF1) & todo.j3$CAMPO==est & !is.na(todo.j3$CAMPO))
...
#Tabla: Promedio de borregos manejados a campo por año
...
tabla3.e <- as.data.frame(cbind(N,PC1,PVS1,RIN1,PVL1,PDF1,CVF1,FC1,LM1,RT1))
...
#Tabla: Promedio de borregos suplementados por año
...
rm(N,PC1,PVS1,RIN1,PVL1,PDF1,CVF1,LM1,RT1,tabla4.e)
...
rm(t10,t11,t12,t16,t17,tabla1.e,tendencia.est,est1)
