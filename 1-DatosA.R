...
#Aqui esta la base de datos sin pedigree de caba?as
#Antes de importar guardar el archivo access como csv

#aqui esta la base de datos de todas las caba?as AACM
...
excel3 <- read.csv('c:/Drive/Data/Merino/A/Pilca/pilca new.csv', sep=';')

todo3 <- subset(excel3, excel3$B =='A')
todo3$B <- droplevels(as.factor(todo3$B))
todo3$CAMPO <- 'Pilc'

todo3$MANEJO[todo3$MANEJO=="Alto"] <- "cabania"
todo3$MANEJO[todo3$MANEJO=="Medio"] <- "cabania"
