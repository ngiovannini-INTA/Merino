...
VC1.j <-VC1.j[,-c(1,3,4,5,7,9,11,12)]
...
todo.j$exa <- apply(todo.j[ ,c("acPC1","acPDF1","acPVL1","acPCD")], 1, mean, na.rm = TRUE)

todo.j <- todo.j[,-c(5:7,14:16,22:24,28:30)]

#Calcula los ?ndices crudos y estandarizados

todo.j$i10crudo <- todo.j$vcNCD*903.2 + todo.j$vcPCD*30.4 + todo.j$vcPVL1*485 + todo.j$vcPDF1*(-88.19) + todo.j$vcPC2*3.2;
todo.j$i02crudo <- todo.j$vcNCD*903.2 + todo.j$vcPCD*30.4 + todo.j$vcPVL1*485 + todo.j$vcPDF1*(-23.2) + todo.j$vcPC2*3.2;
...
ab <- todo.j2[todo.j2$AN==2010,] #A?o Base (lo cambi? en 2020)
